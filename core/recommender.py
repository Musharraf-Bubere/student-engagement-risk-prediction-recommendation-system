...
def generate_recommendations(risk_probability, student_input):
    ...
    if student_input["early_click"] < THRESHOLD["early_engagement"]["min_early_click"]:
    ...
    if student_input["first_activity_day"] > 0:
        observations.append("Late course start comapared to peers")
        actions.append("Follow a structured 7-day recovery plan")

    if risk_probability >= THRESHOLD["risk_levels"]["high"]:
